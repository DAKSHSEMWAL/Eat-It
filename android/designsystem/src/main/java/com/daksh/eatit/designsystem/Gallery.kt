    ...
    Column(modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(24.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            listOf(MaterialTheme.colorScheme.primary, MaterialTheme.colorScheme.secondaryContainer,
                MaterialTheme.colorScheme.tertiary, MaterialTheme.colorScheme.surfaceVariant).forEach {
                Box(Modifier.size(48.dp).background(it, MaterialTheme.shapes.medium))
            }
        }
        ...
        EatItEmptyState("A little room for something good", "Your cart is empty. Explore the menu to get started.", actionLabel = "Explore menu")
        EatItErrorState("We couldn’t load the menu. Check your connection.", {})
    }
}
@Preview(name = "Light", showBackground = true, widthDp = 390)
@Composable private fun LightGallery() { EatItTheme(false) { Surface { EatItGallery() } } }
@Preview(name = "Dark", showBackground = true, widthDp = 390)
@Composable private fun DarkGallery() { EatItTheme(true) { Surface { EatItGallery() } } }
@Preview(name = "Large text", showBackground = true, widthDp = 390, fontScale = 2f)
@Composable private fun LargeTextGallery() { EatItTheme { Surface { EatItGallery() } } }
